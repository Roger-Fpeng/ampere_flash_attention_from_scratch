...
#include <utility>
#include <vector>
#include "afa_cuda_utils.cuh"
#include "afa_config.cuh"
#include "afa_preset_kernels.cuh"

using namespace afa;

AFAForwardKernelConfig py_to_cpp_kernel_config(const py::object &py_cfg) {
    return AFAForwardKernelConfig{
...
flash_attention_forward(const py::object &py_cfg, const torch::Tensor &TQ,
    ...
    const AFAForwardKernelConfig cfg{py_to_cpp_kernel_config(py_cfg)};
    ...
    AFAForwardParams args{TQ.data_ptr(), TK.data_ptr(), TV.data_ptr(),
    ...
    // 每个 CTA 中含有 blockDim 个线程，处理一个 Q 的 tile block, 一个 head 和 batch 中的一个的计算
