...
template <bool async>
AFA_DEVICE void cp_async_commit() {
    if constexpr (async) {
...
template <bool async>
AFA_DEVICE_CONSTEXPR void cp_async_commit_and_wait_all() {
    if constexpr (async) {
...
AFA_DEVICE void cp_async(T *smem_to, T *gmem_from) {
...
// Each thread load 4 * 32bits(4 * 2 fp16/bf16 value).
template <typename T>
AFA_DEVICE void ldmatrix_x4(T *load_from, uint32_t &a1, uint32_t &a2,
...
template <typename T>
AFA_DEVICE void ldmatrix_x4_transpose(T *load_from, uint32_t &a1, uint32_t &a2,
                                     uint32_t &a3, uint32_t &a4) {
    uint32_t smem_ptr = __cvta_generic_to_shared(load_from);
