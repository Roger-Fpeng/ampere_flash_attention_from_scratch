...
template <typename AMatrixLDST_, typename BMatrixLDST_, typename CMatrixLDST_, int col_fragments_per_matmul,
...
struct GEMM {
    ...
    static constexpr int TotalKTiles = col_fragments_per_matmul;
...
AFA_DEVICE_CONSTEXPR void warp_fragment_mma_f32_accum(
    ...
    // 会一次加载多个 fragment 数据到寄存器中，然后进行 mma 计算，如果一次加载的 fragment 过多，计算和通信重叠的部分就小
    // 为什么在乘法中累加寄存器 c 不变: 实际计算的是 tile 矩阵的一行乘以一列的结果，累加寄存器 c 不变（只不过过个 col 的 fragment
    AFA_UNROLL
...
AFA_DEVICE_CONSTEXPR void matmul(typename GEMM::A_t &A, typename GEMM::B_t &B,
                                typename GEMM::C_t &C) {
